...
import 'package:ChaatBar/view/component/category_component.dart';
import 'package:ChaatBar/view/component/menu_screen_category_component.dart';
import 'package:ChaatBar/view/component/no_item_available.dart';
import 'package:ChaatBar/view/component/view_cart_container.dart';
...
import '../../../model/response/rf_bite/featuredListResponse.dart';
...
class _MenuScreenState extends State<MenuScreen>
                                                ...
                                                ? Wrap(
                                                    ...
                                                    children:
                                                      ...
                                                      return ProductComponent(
                                                          ...
                                                          screenHeight:
                                                              screenHeight,
                                                          showFavIcon: detailType == "featured" ? false : true,
                                                          onAddTap: () {
                                                          ...
                                                          onPlusTap: () {
                                                            if ((item.productSizesList == "[]" ||
                                                                ...
                                                                (item.addOn ==
                                                                    ...
                                                                    item.addOn ==
                                                                        null) && item.isBuy1Get1 == false) {
                                                              print("item.isBuy1Get1 :: ${item.isBuy1Get1}");
                                                              setState(() {
                                                                if (item.quantity <=
                                                                    int.parse(
                                                                        "${item.qtyLimit}")) {
  ...
  void _setFavorite( int? productId) async {
    ...
    if (!isConnected) {
      ...
      MarkFavoriteRequest request =
          MarkFavoriteRequest(customerId: customerId, productId: productId ?? 0,vendorId: vendorId);
      await Future.delayed(Duration(milliseconds: 2));
      await Provider.of<MainViewModel>(context, listen: false)
  ...
  void _fetchFavoritesData() async {
    ...
    if (!isConnected) {
      ...
      await Future.delayed(Duration(milliseconds: 2));
      await Provider.of<MainViewModel>(context, listen: false)
          .fetchFavoritesProductList("/api/v1/app/products/get_favourites",request);
      ApiResponse apiResponse =
          Provider.of<MainViewModel>(context, listen: false).response;
  ...
  Future<void> initializeDatabase() async {
    ...
    if (detailType == "menu" || detailType == "category") {
      setState(() {
