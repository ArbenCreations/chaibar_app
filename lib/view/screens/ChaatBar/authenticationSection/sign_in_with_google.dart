...
import '../../../../model/response/rf_bite/signInResponse.dart';
...
Future<void> _signInWithGoogle(User? user, String idToken,BuildContext context,String deviceToken) async {
  //Navigator.pushNamed(context, "/VendorScreen");
  const maxDuration = Duration(seconds: 2);
  if (user != null) {
    SignUpWithGoogleRequest request = SignUpWithGoogleRequest(
        customer: CustomerSignUpWithGoogle(email: "${user.email}",
            idToken: idToken,
...
Future<void> getSignInResponse(BuildContext context,
    ApiResponse apiResponse,User?user) async {
...
Future<void> _saveChanges(String phoneNo,BuildContext context,String userId,User? user) async {
    hideKeyBoard();
...
void _showModal(BuildContext context, User? user,String userId) {
  showDialog<void>(
      context: context,
